...
const taskCompletionSlice = createSlice({
  ...
  reducers: {
    ...
    findCompletionsByTaskId: (state, action: PayloadAction<{ taskId: string }>) => {
      ...
      findCompletionsByTaskAndProfielId: (state, action: PayloadAction<{ taskId: string, profileId:string }>) => {
        const { taskId,profileId } = action.payload;
        const foundCompletion = state.taskCompletions.find((completion) => completion.taskId=== taskId && completion.profileId === profileId);
        if (foundCompletion) {
          state.taskCompletions = [foundCompletion]; 
        } else {
        }
